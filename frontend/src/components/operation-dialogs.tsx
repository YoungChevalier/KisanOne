...
function BookingForm({ onDone }: { onDone: () => void }) {
  ...
  const [crops, setCrops] = useState<api.CropConfig[]>([]);
  ...
  useEffect(() => {
    ...
    api.listCropConfigs().then(c => setCrops(c.filter(x => x.status === "ACTIVE"))).catch(() => {});
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!cropCode) throw new Error("Please select a crop");
  ...
  return <>
    ...
    <Field label="Crop / फसल">
      <Select value={cropCode} onChange={setCropCode} options={crops.map(c => ({ value: c.cropCode, label: `${c.cropName} (${c.cropCode})` }))} placeholder="Select crop..." />
    </Field>
